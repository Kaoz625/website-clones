...
import { ocrImage, inferElementType } from './ocr.js';
...
const server = new Server({ name: 'uivision-mcp', version: '1.0.0' }, { capabilities: { tools: {} } });
    ...
    try {
        switch (name) {
            case 'take_screenshot': {
                const { url, fullPage, width, height, outputName } = args;
            ...
            case 'click_text': {
                const { text, screenshotPath } = args;
            ...
            case 'image_search': {
                const { templatePath, sourcePath } = args;
            ...
            case 'describe_layout': {
                ...
                for (const el of elements) {
                    if (!elementsByType[el.elementType])
                        elementsByType[el.elementType] = [];
